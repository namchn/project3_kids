...
<body>
   ...
   <table class="table" style="width: 700px">
      <tr><th style="font-size: 30px">${nb.title}</th></tr>
      <tr><td>${nb.id} | 조회 ${nb.viewcount} | ${nb.time}</td></tr>
      <tr><td style="padding-top: 50px;padding-bottom: 50px;">
         ${fn:replace(nb.content,cn,br) }
      </td></tr>
      <tr><td>첨부파일 : <a href="${pageContext.request.contextPath }/download/down?name=${nb.files}" style="font-size: 15px;">${nb.files}</a></td></tr>
      <tr>
         <td>
            <div style="position: relative; left: 485px;">
            <a href="${pageContext.request.contextPath}/boardNotice/noticeList?sort=${sort}" class="btn btn-primary" style="color: white;">글목록</a>
            <c:if test="${sessionScope.id==nb.id}">
               <a href="${pageContext.request.contextPath}/boardNotice/noticeEditForm?notice_num=${nb.notice_num}&sort=${sort}" class="btn btn-primary" style="color: white;">글수정</a>
               <a href="${pageContext.request.contextPath}/boardNotice/noticeDelete?notice_num=${nb.notice_num}&sort=${sort}" class="btn btn-primary" style="color: white;">삭제</a>
            </c:if>            
            </div>
         </td>
       </tr>
